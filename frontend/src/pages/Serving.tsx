...
export default function Serving() {
  ...
  const active = provider === "ollama" ? !!state?.ollama.managed_model : provider === "vllm" ? !!state?.vllm.active : !!state?.transformers.active;
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      <Card><CardHeader><CardTitle>Serving provider</CardTitle></CardHeader><CardContent className="space-y-4">
        <Select aria-label="Serving provider" disabled={busy} value={provider} onChange={e => setProvider(e.target.value)}><option value="transformers">Transformers — managed local runtime</option><option value="vllm">vLLM — dedicated service</option><option value="ollama">Ollama — installed compatible models</option></Select>
        {provider === "transformers" ? <BaseModelPicker value={model} onChange={setModel} /> : provider === "vllm" ? <>
          <Badge variant={state?.vllm.active ? "success" : "warning"}>{state?.vllm.active ? "vLLM ready" : "vLLM stopped"}</Badge>
          <p className="text-sm text-muted-foreground">{state?.vllm.active ? `${state.vllm.models.length} served model(s) discovered through /v1/models.` : state?.vllm.guidance ?? "Checking vLLM…"}</p>
        </> : <>
        ...
        <div className="flex gap-2"><Button disabled={provider === "vllm" || busy || active || (provider === "ollama" ? !state?.ollama.running || !ollamaModel : !state?.transformers.available || !model.trim())} onClick={() => start.mutate()}>{provider === "vllm" ? "Managed by Docker profile" : start.isPending ? "Loading model…" : "Serve Model"}</Button>
          <Button variant="danger" disabled={provider === "vllm" || busy || !active} onClick={() => stop.mutate()}>{stop.isPending ? "Stopping…" : "Stop Server"}</Button></div>
      ...
      <Card><CardHeader><CardTitle>Endpoint & status</CardTitle></CardHeader><CardContent className="space-y-3">
        ...
        <p className="break-all font-mono text-sm">{provider === "ollama" ? state?.ollama.endpoint : provider === "vllm" ? state?.vllm.endpoint : state?.transformers.endpoint ?? "Start a model to expose its API"}</p>
        <Button variant="outline" size="sm" disabled={!active} onClick={async () => {
          try { await navigator.clipboard.writeText(provider === "ollama" ? state!.ollama.endpoint : provider === "vllm" ? state!.vllm.endpoint : state!.transformers.endpoint!); toast.success("Endpoint copied"); }
